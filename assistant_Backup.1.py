...
# -- TTS: Kokoro offline synthesis → WAV → winsound playback -----------------
...
def _tts_worker():
    ...
    try:
        ...
        print("✅ Kokoro TTS ready — British voice (George) active.")
    except Exception as e:
...
def mute_jarvis_instantly():
    ...
    try:
        subprocess.run(['powershell', '-NoProfile', '-Command',
                        '[System.Media.SoundPlayer]::new().Stop()'],
                       capture_output=True)
    except Exception:
        pass
...
class BrowserAgent:
    ...
    def _kill_profile19_chrome(self):
        """Kill only Chrome processes holding a lock on Profile 19 — leaves all other Chrome windows open."""
        if PSUTIL_OK:
            killed = 0
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    if (proc.info['name'] or '').lower() == 'chrome.exe':
                        ...
                        if 'Profile 19' in cmdline:
                            proc.kill()
                            killed += 1
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    pass
            if killed:
                print(f"🧹 Closed {killed} Chrome process(es) using Profile 19")
        else:
    ...
    def _start(self):
        ...
        # Kill only Chrome processes that are already using Profile 19 — leave the user's other Chrome windows alone
        self._kill_profile19_chrome()
        time.sleep(0.5)
